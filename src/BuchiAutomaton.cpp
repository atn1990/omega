...
void BuchiAutomaton::Print() const {
  ...
  if (verbose == std::to_underlying(OutputType::Outfile)) {
    ...
    std::print("# BEGIN TRANSITIONS\n");
    Automaton::Print();
    std::print("# END TRANSITIONS\n");
    ...
    if (verbose > std::to_underlying(OutputType::General)) {
      for (auto i : omega::set_bits(initial_states)) {
        std::print("  {:{}}", i, width);
      }
    ...
    std::print("# FINAL\n");
    if (verbose > std::to_underlying(OutputType::General)) {
      for (auto i : omega::set_bits(final_states)) {
        std::print("  {:{}}", i, width);
      }
...
void BuchiAutomaton::Clean() {
  ...
  dbg(OutputType::Debug, std::print("Vertices Removed: {}\n\n", num_removed));
...
void BuchiAutomaton::Reachable() {
  ...
  dbg(OutputType::Debug, std::print("Reachable: {}\n\n", reachable));

  reachable.flip();

  if (reachable.any()) {
    for (auto i : omega::set_bits(reachable)) {
      auto v = boost::vertex(i, graph);
...
void BuchiAutomaton::ProjectLabel() {
  dbg(OutputType::General, std::print("# ProjectLabel()\n"));
...
// Display a path in a graph ending at a target vertex `v` using the vertices in the predecessor list `pred`, generated from a breadth first search traversal of the underlying transition graph.
void BuchiAutomaton::FindPath(
  ...
  const auto edge_width = binary_digits(num_alphabet-1);

  if (pred[v] == v) {
    return;
  }

  FindPath(pred, pred[v], path);

  const auto width = binary_digits(num_alphabet-1);
  for (auto [itr, end] = boost::in_edges(v, graph); itr != end; ++itr) {
    ...
    std::print("({:{}}, {:0{}b})  -->  {:{}}\n", index[s], vertex_width, symbol[*itr], edge_width, index[v], vertex_width);

    return;
  }
}

// Find a cycle within a strongly connected component containing the source vertex s.
void BuchiAutomaton::FindCycle(
  ...
  if (loop) {
    ...
    std::print("({:{}}, {:0{}b})  -->  {:{}}\n", vertex_index[s], vertex_width, symbol[e], edge_width, vertex_index[s], vertex_width);

    return;
  }

  // Since s is in a strongly connected component and it doesn't have a self-loop, there must exist some neighbor t in the same component.
  graph_t::vertex_descriptor t = s;
...
bool BuchiAutomaton::Empty() {
  ...
  dbg(OutputType::Debug, std::print("Strongly Connected Components: {}\n", scc));
  ...
  const auto vertex_width = decimal_digits(num_vertices-1);
  const auto scc_width = decimal_digits(scc-1);
  auto vertex_index = boost::get(boost::vertex_index, graph);
  ...
  for (auto& list : component_lists) {
    ...
    for (auto& u : list) {
      ...
      dbg(OutputType::Debug, std::print("component({:{}})  =  {:{}}\n", k, vertex_width, component[k], scc_width));
    ...
    if (trivial) {
      ...
      // A component of exactly one final state is trivial unless the state has a self loop.
      if (!loop) {
        component[vertex_index[u]] = TRIVIAL;
      }
    }
  ...
  for (auto i : omega::set_bits(initial_states)) {
    ...
    for (auto j : omega::set_bits(final_states)) {
...
std::unique_ptr<BuchiAutomaton> BuchiAutomaton::RabinScott(
  ...
  dbg(OutputType::Quiet, {
    std::print("# STATES GENERATED {}\n", num_states);
    std::print("# STATES HASHED    {}\n", hash.size());
    std::print("# EMPTY STATES     {}\n", num_empty);
    std::print("# TRANSITIONS      {}\n\n", dfa_map.size());
  });

  dbg(OutputType::General, std::print("# STATES\n"));
...
void BuchiAutomaton::Minimize() {
  ...
  dbg(OutputType::General, std::print("# Minimize()\n\n"));

  auto width = decimal_digits(num_vertices-1);
  ...
  for (auto i = 0U; i < num_vertices; i++) {
    ...
    dbg(OutputType::General, std::print("{:{}}  index = {:{}}    {}\n", i, width, E[i], width, boost::to_string(final_states[i])));
  }
  if (false) {
  ...
  auto index = boost::get(boost::vertex_index, graph);
  auto label = boost::get(boost::edge_name, graph);
  width = binary_digits(num_alphabet-1);
  ...
  while (!equivalent) {
    ...
    for (auto s = 0UL; s < num_alphabet; s++) {
      ...
      for (auto j = 0UL; j < num_vertices; j++) {
        ...
        dbg(OutputType::General, std::print("  {:{}}", temp[j], width));
      }
      ...
      for (auto j = 0UL; j < num_vertices; j++) {
        ...
        temp[j] = -1;
        dbg(OutputType::General, std::print("  {:{}}", E[j], width));
      }
...
std::unique_ptr<BuchiAutomaton> Intersection(const BuchiAutomaton& A, const BuchiAutomaton& B) {
  ...
  const auto A_width = decimal_digits(A.num_vertices-1);
  ...
  std::unordered_map<std::tuple<int_type, int_type, NodeType>, graph_t::vertex_descriptor> map;

  std::queue<std::tuple<int_type, int_type, NodeType, graph_t::vertex_descriptor>> queue;

  dbg(OutputType::Debug, std::print("# N(A) = {}, N(B) = {}, S = {}\n# Initial States\n", A.num_vertices, B.num_vertices, C->num_alphabet));

  for (auto i : omega::set_bits(A.initial_states)) {
    for (auto j : omega::set_bits(B.initial_states)) {
      ...
      map.insert({{i, j, NodeType::Initial}, u});

      // Fill in the graph as a BFS from the initial states.
      queue.push({i, j, NodeType::Initial, u});

      // The state (i, j, NodeType::Initial) is both initial and final.
      state_C[u] = NodeType::Initial;

      dbg(OutputType::Debug, std::print("({:{}}, {:{}}, {})\n", i, A_width, j, B_width, print_state(NodeType::Initial)));
    }
  }
  ...
  while (!queue.empty()) {
    auto [idx_source_A, idx_source_B, component, u_C] = queue.front();
    queue.pop();

    dbg(OutputType::Debug, std::print("({:{}}, {:{}}, {})\n", idx_source_A, A_width, idx_source_B, B_width, print_state(component)));

    auto u_A = boost::vertex(idx_source_A, A.graph);
    auto u_B = boost::vertex(idx_source_B, B.graph);

    if (component == NodeType::Initial) {
      // NodeType::Initial -> NodeType::Final1 on next input
      component = NodeType::Final1;
    } else if (component == NodeType::Final1 && A.final_states[idx_source_A]) {
      // NodeType::Final1 -> FinalType::Final2 when a final state of A is seen
      component = NodeType::Final2;
    } else if (component == NodeType::Final2 && B.final_states[idx_source_B]) {
      // NodeType::Final2 -> NodeType::Initial when a final state of B is seen
      component = NodeType::Initial;
    }
    ...
    for (auto [itr_A, end_A] = boost::out_edges(u_A, A.graph); itr_A != end_A; ++itr_A) {
      auto symbol = label_A[*itr_A];

      auto target_A = boost::target(*itr_A, A.graph);
      auto idx_target_A = index_A[target_A];

      // BGL_FORALL_OUTEDGES(u_B, e_B, B.graph, graph_t) {
      for (auto [itr_B, end_B] = boost::out_edges(u_B, B.graph); itr_B != end_B; ++itr_B) {
        if (symbol != label_B[*itr_B]) {
          continue;
        }

        auto target_B = boost::target(*itr_B, B.graph);
        auto idx_target_B = index_B[target_B];

        dbg(OutputType::Debug, std::print("  {:0{}b}  -->  ({:{}}, {:{}}, {})", symbol, C_width, idx_target_A, A_width, idx_target_B, B_width, print_state(component)));

        auto tuple = std::make_tuple(idx_target_A, idx_target_B, component);
        ...
        if (itr == map.end()) {
          ...
          if (component == NodeType::Initial) {
            state_C[v] = NodeType::Final;
          } else {
            state_C[v] = NodeType::None;
          }

          itr = map.insert({tuple, v}).first;

          queue.push({idx_target_A, idx_target_B, component, v});
        }
...
std::unique_ptr<BuchiAutomaton> DisjointUnion(const BuchiAutomaton& A, const BuchiAutomaton& B) {
  ...
  auto M = std::make_unique<BuchiAutomaton>(num_alphabet, num_vertices);

  auto label = boost::get(boost::edge_name, M->graph);
  auto state = boost::get(boost::vertex_name, M->graph);
  ...
  for (auto i = 0UL; i < A.num_vertices; i++) {
    ...
    for (auto [itr, end] = boost::out_edges(u_A, A.graph); itr != end; ++itr) {
      ...
      auto v = boost::vertex(index, M->graph);
  ...
  for (auto i = 0UL; i < B.num_vertices; i++) {
    auto u = boost::vertex(i + A.num_vertices, M->graph);
    auto u_B = boost::vertex(i, B.graph);

    if (B.initial_states[i]) {
      M->initial_states.set(i + A.num_vertices);
    }

    if (B.final_states[i]) {
      M->final_states.set(i + A.num_vertices);
    }
